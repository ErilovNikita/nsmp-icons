...
Цвет Vue-компонента можно задать параметром `color`:

```vue
<template>
	<AddCallIcon color="#e11d48" width="20" height="20" />
	<ZoomInIcon color="var(--accent-color)" />
</template>
```

Без `color` используется текущее значение CSS `color` родителя. Компоненты принимают стандартные HTML-атрибуты и импортируются отдельно, поэтому неиспользуемые иконки не попадают в bundle. Для этого варианта приложение должно использовать Vue 3.
